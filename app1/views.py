...
def stud_dash(request):
	if 'User_type' in request.session:
		if request.session['User_type'] == "Student" or request.session['User_type'] == "student":
			context = {}
...
def pdf_generator(request):
	...
	if(d0.days > 0):
		...
		fine = str(0)
	str1 = str(obj_request.requestedBy) + " has returned book named " + str(obj_request.name) + " on " + str(date.today()) + " with fine of "+ fine + " Rupees"
...
def del_books(request):
	if request.method=='POST':
		...
		context = {}
		if models.books.objects.filter(name=Name).exists():
			instance = models.books.objects.get(name=Name)
			instance.delete()
			response = HttpResponseRedirect('/librarian_dash')
		else:
			message = "Book doesn't exist"
			context['message'] = message
			response = render(request,"del_books.html",context)
	else:
		response = render(request,'del_books.html',{})
	return response
